...
class TestShipments(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.shipments_instance = shipments.Shipments("path/to/your/root/", is_debug=True)

    def test_get_suppliers_by_id(self):
        with open('data/suppliers.json') as file:
            expected_data = json.load(file)
        
        result = self.shipments_instance.get_supplier_by_id(1)  # assuming `get_supplier_by_id` is the actual method
    ...
    def test_add_transfers(self):
        ...
        self.assertIsNotNone(result, "Shipment with ID 60 should be added but returned None")
